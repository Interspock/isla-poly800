#!/usr/bin/env python3
"""Validate Poly-800 MkI factory data and generate LV2 preset Turtle.

The canonical CSV contains only factual program parameter values recovered from
preserved factory sources. Original cassette audio, scans, firmware and binary
dumps are deliberately not vendored in this repository.
"""
...
PARAMS = [
    (11, "p11_dco1_octave", 1, 3), (12, "p12_dco1_waveform", 1, 2),
    (13, "p13_dco1_h16", 0, 1), (14, "p14_dco1_h8", 0, 1),
    (15, "p15_dco1_h4", 0, 1), (16, "p16_dco1_h2", 0, 1),
    (17, "p17_dco1_level", 0, 31), (18, "p18_dco_mode", 1, 2),
    (21, "p21_dco2_octave", 1, 3), (22, "p22_dco2_waveform", 1, 2),
    (23, "p23_dco2_h16", 0, 1), (24, "p24_dco2_h8", 0, 1),
    (25, "p25_dco2_h4", 0, 1), (26, "p26_dco2_h2", 0, 1),
    (27, "p27_dco2_level", 0, 31), (31, "p31_dco2_interval", 0, 12),
    (32, "p32_dco2_detune", 0, 3), (33, "p33_noise", 0, 15),
    (41, "p41_vcf_cutoff", 0, 99), (42, "p42_vcf_resonance", 0, 15),
    (43, "p43_vcf_keytrack", 0, 2), (44, "p44_vcf_polarity", 1, 2),
    (45, "p45_vcf_env", 0, 15), (46, "p46_vcf_trigger", 1, 2),
    (48, "p48_chorus", 0, 1),
    (51, "p51_deg1_attack", 0, 31), (52, "p52_deg1_decay", 0, 31),
    (53, "p53_deg1_breakpoint", 0, 31), (54, "p54_deg1_slope", 0, 31),
    (55, "p55_deg1_sustain", 0, 31), (56, "p56_deg1_release", 0, 31),
    (61, "p61_deg2_attack", 0, 31), (62, "p62_deg2_decay", 0, 31),
    (63, "p63_deg2_breakpoint", 0, 31), (64, "p64_deg2_slope", 0, 31),
    (65, "p65_deg2_sustain", 0, 31), (66, "p66_deg2_release", 0, 31),
    (71, "p71_deg3_attack", 0, 31), (72, "p72_deg3_decay", 0, 31),
    (73, "p73_deg3_breakpoint", 0, 31), (74, "p74_deg3_slope", 0, 31),
    (75, "p75_deg3_sustain", 0, 31), (76, "p76_deg3_release", 0, 31),
    (81, "p81_mg_frequency", 0, 15), (82, "p82_mg_delay", 0, 15),
    (83, "p83_mg_dco", 0, 15), (84, "p84_mg_vcf", 0, 15),
]
...
def read_verified(catalog: dict[int, str]) -> list[tuple[int, list[int]]]:
    with DATA.open(newline="", encoding="utf-8") as fh:
        ...
        if reader.fieldnames != EXPECTED_DATA_HEADER:
            raise ValueError("verified CSV header mismatch; expected: " + ",".join(EXPECTED_DATA_HEADER))
        result = []
        seen = set()
        previous = 0
        for line, row in enumerate(reader, start=2):
            ...
            for number, _symbol, lo, hi in PARAMS:
                ...
                if not lo <= value <= hi:
                    raise ValueError(f"program {program}: p{number}={value} outside {lo}..{hi}")
                values.append(value)
            seen.add(program)
            previous = program
...
def render(catalog: dict[int, str], rows: list[tuple[int, list[int]]]) -> str:
    ...
    for program, values in rows:
        ...
        lines.extend([
            f"<{uri}>", "    a pset:Preset ;", f"    lv2:appliesTo <{PLUGIN}> ;",
            f"    pset:bank <{BANK}> ;", f'    rdfs:label "{ttl_escape(label)}" ;', "    lv2:port",
        ])
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="validate canonical factory input without writing output")
    parser.add_argument("--require-complete", action="store_true", help="fail unless all 64 programs are verified")
    parser.add_argument("--output", type=pathlib.Path, help="write generated LV2 Turtle to this path")
    ...
    try:
        ...
        if args.output:
            ...
            args.output.write_text(render(catalog, rows), encoding="utf-8")
        elif not args.check:
            sys.stdout.write(render(catalog, rows))
    except (OSError, ValueError) as exc:
        print(f"factory preset validation failed: {exc}", file=sys.stderr)
        return 1
    print(f"factory catalog: 64 programs; verified parameter rows: {len(rows)}/64", file=sys.stderr)
    return 0
